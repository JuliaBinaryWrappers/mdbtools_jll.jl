# Autogenerated wrapper script for mdbtools_jll for x86_64-unknown-freebsd11.1
...
const libmdb_splitpath = ["lib", "libmdb.so"]
...
const libmdb = "libmdb.so.2"


# Relative path to `libmdbsql`
const libmdbsql_splitpath = ["lib", "libmdbsql.so"]
...
# This must be `const` so that we can use it with `ccall()`
const libmdbsql = "libmdbsql.so.2"
